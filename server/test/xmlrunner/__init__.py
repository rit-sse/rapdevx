...
class _XMLTestResult(_TextTestResult):
    ...
    def _prepare_callback(self, test_info, target_list, verbose_str,
        ...
        def callback():
            ...
            if self.showAll:
                ...
                self.stream.write(str(short_str))
    ...
    def startTest(self, test):
        ...
        if self.showAll:
            self.stream.write('  ' + str(self.getDescription(test)))
            self.stream.write(" ... ")
    ...
    def printErrorList(self, flavour, errors):
        ...
        for test_info in errors:
            self.stream.writeln(str(self.separator1))
            self.stream.writeln('%s [%.3fs]: %s' % \
                (flavour, test_info.get_elapsed_time(), \
                test_info.get_description()))
            self.stream.writeln(str(self.separator2))
    ...
    def generate_reports(self, test_runner):
        ...
        for suite, tests in all_results.items():
            ...
            if type(test_runner.output) is str:
                ...
                try:
                    report_file.write(str(xml_content))
                finally:
                    report_file.close()
            else:
                # Assume that test_runner.output is a stream
                test_runner.output.write(str(xml_content))
...
class XMLTestRunner(TextTestRunner):
    ...
    def run(self, test):
        ...
        try:
            ...
            self.stream.writeln(str(result.separator2))
